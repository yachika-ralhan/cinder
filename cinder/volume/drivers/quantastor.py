...
from cinder.volume.drivers.quantastor_api import QuantastorClient


LOG = logging.getLogger(__name__)

QS_OPTS = [
    cfg.StrOpt('san_ip', default=None,
               help='The IP/FQDN of the Quantastor system '
                    'being used for storage'),

    cfg.StrOpt('san_pool_id',
               default=None,
               help='The ID of the Quantastor pool or tier to use.'),

    cfg.StrOpt('san_login',
               default=None,
               help='The account on the Quantastor system '
                    'to be used by cinder'),

    cfg.StrOpt('san_password',
               default=None,
               help='The password for the Quantastor account being used',
               secret=True)]
...
class QuantaStorDriver(driver.ISCSIDriver):
    ...
    def do_setup(self, context):
        ...
        module = {
            'params': {
                'quantastor_hostname': self.configuration.san_ip,
                'quantastor_username': self.configuration.san_login,
                'quantastor_password': self.configuration.san_password
            }}
        module = DotDict(module)
        self.client = QuantastorClient(module)
    ...
    def check_for_setup_error(self):
        ...
        try:
            ...
            LOG.debug("connected to Quantastor system =%s"
                      "Found pool with ID =%s"
                      " named %s", sys_info._name, pool_info._id,
                      pool_info._name)
    ...
    def create_volume(self, volume):
        ...
        provisionable_id = self.configuration.san_pool_id
    ...
    def create_snapshot(self, snapshot):
        ...
        LOG.debug("Snapshot =%s was successfully created", qs_snap)
    ...
    def delete_snapshot(self, snapshot):
        ...
        LOG.debug("Snapshot =%s was successfully deleted", vol)
    ...
    def initialize_connection(self, volume, connector):
        ...
        portal = self.configuration.san_ip + ":3260"
    ...
    def _update_volume_stats(self):
        ...
        stats["volume_backend_name"] = 'qstor'
        ...
        if pool._size is not None:
            stats['total_capacity_gb'] = int(self.b_to_gb_size(pool._size))
        if pool._freeSpace is not None:
            stats['free_capacity_gb'] = int(self.b_to_gb_size(pool._freeSpace))
    ...
    def extend_volume(self, volume, new_size):
        ...
        out = self.client.storage_extend_volume(volume['name'],
                                                self.configuration.san_pool_id,
    ...
    def b_to_gb_size(cls, size_b):
        """converting b to gb"""
        return str(int(size_b) / (units.Ki ** 3))
    ...
    def qs_pool_get(self):
        """pool info"""
        pool = self.client.storage_pool_get(self.configuration.san_pool_id)
